	...
	mounted() {
	  ...
	  methods:{
	    ...
	    getList() {
	        let vm = this;
	        vm.listLoading = true;
	        global.post(api.getClassSchedule,{siClCode:vm.classInfo.clCode},{header:{"Content-type":"application/json"}},function(res){
	                let data = res.data;
	               if(res.code == 200){ 
	                    vm.list = data.list;
	    ...
	    getClassInfo() {
	      let vm = this;
	      vm.listLoading = true;
	      global.post(api.selectByTcNum,{roleType:2,tcNum:vm.userInfo.tcUserid},{header:{"Content-type":"application/json"}},function(res){
	              let data = res.data;
	             if(res.code == 200){
