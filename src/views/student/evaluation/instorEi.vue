<template>
  <div class="app-container" v-loading.body="loading" element-loading-text="拼命加载中">
      <div style="margin-left: 25px;margin-right: 30px;margin-top: 10px;line-height: 30px; color: #48576a;">
        ...
        <div style="margin-top:30px; border-bottom: 1px solid rgb(238, 246, 244);font-weight: bold;font-size: 15px;color: #009751;">辅导员信息</div>

        <div class="instorInfo"  v-if="classInfo.instor != undefined">
          <el-row>
          ...
          <el-row>
            <el-col :span="8">工号：{{classInfo.instor.tcNum}}</el-col>
            <el-col :span="8"> 姓名：{{classInfo.instor.tcName}}</el-col>
            <el-col :span="8">联系方式：{{classInfo.instor.tcTel}}</el-col>
          </el-row>
        </div>
        <div v-if="classInfo.instor == undefined" style="text-align: center;margin-top: 5px;">暂无辅导员信息</div>
        <div style="margin-top:30px;margin-bottom:15px; border-bottom: 1px solid rgb(238, 246, 244);font-weight: bold;font-size: 15px;color: #009751;">辅导员教学评价</div>
          <el-table
            :data="eiContent"
            border
            style="width: 100%">
...
export default {
  data() {
    return {
        ...
        loading: true,
        otherAdvise:"",
        classInfo:{instor:{tcNum:0,tcName:"",tcTel:""},dept:{}},
        instorEi:0,
        eiContent: [{
           type: '工作方面',
  ...
  mounted() {
    let vm = this;
    vm.instorEi = vm.userInfo.stuInstorEi;
    vm.getClassInfo();
    if(vm.instorEi == 1){
      vm.$notify({
          title: '教学评价通知',
  ...
  methods: {
    ...
    submit(){
      ...
      evalution.eiTarget = vm.classInfo.clInstor;
    ...
    updateStudentInfo(){
      let vm = this;
      var params = {stuUserid:vm.userInfo.stuUserid,stuInstorEi:1}
